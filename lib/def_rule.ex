defmodule DefRule do
  ...
  def crack_fact_template_name_from_ast_node(
        {:=, _, [{:%, _, [{:__aliases__, meta, parts} | _]} | _]} = node,
        env
      ) do
    ...
    parts_as_atoms =
      case resolved do
        atom when is_atom(atom) ->
          Module.split(atom) |> Enum.map(&String.to_existing_atom/1)

        {:__aliases__, _, new_parts} ->
          new_parts
  ...
  def crack_fact_template_name_from_ast_node(
        {:%, _, [{:__aliases__, meta, parts} | _]} = node,
        env
      ) do
    resolved = Macro.expand({:__aliases__, meta, parts}, env)
  ...
  def crack_fact_template_names_from_ast_node(closure_function_head, env)
  ...
  def crack_function_info_from_ast_node(
        {:fn, _,
         [
           {:->, _,
            [closure_function_head | [{_, _, [lhs_evaluation_body | rhs_execution_body]} | _]]}
           | _
         ]}
      ) do
  ...
  defmacro def_rule(rules, rule_name, rule) do
    {closure_function_head, lhs_evaluation_body, rhs_execution_body} =
      crack_function_info_from_ast_node(rule)
    ...
    fact_template_name_frequencies =
      fact_template_names
      |> Enum.frequencies()
    ...
    quote do
      ...
      structs =
        unquote(Macro.escape(fact_template_names))
        |> Enum.map(fn
          {:__aliases__, _, parts} ->
